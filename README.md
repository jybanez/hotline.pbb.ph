...
Hotline uses the shared realtime socket for discovery, presence, call flow, media notifications, and citizen location.

Primary rooms:
- `presence.global.hotline`: citizen/operator discovery, operator availability presence, realtime connection health, and citizen location updates.
...
- `citizen.operator.available.request`: citizen asks online available operators to respond.
- `citizen.operator.available.response`: an available operator responds to discovery.
- `citizen.call.request`: citizen sends the selected operator a call request.
- `citizen.location.updated`: citizen location update published during an active call.
...
Citizen call routing:
1. Citizen captures the best available location and broadcasts `citizen.operator.available.request`.
2. Available operators respond with `citizen.operator.available.response`.
3. The first usable operator response receives the call request.
4. If the operator does not answer within `settings.call_timeout_seconds`, the citizen excludes that operator and retries discovery.
5. If all available operators have already been attempted and none answered, the citizen is informed that operators are busy and should try again later.
...
- show citizen utility actions such as history and availability
- act as the background surface under citizen overlays
...
- allow citizen hang-up before answer
...
- allow citizen review without leaving Citizen Home
...
Citizen overlay priority:
1. Citizen Home is always the base surface.
...
- Dark vector map style configured through `public/hotline.json` and `public/maps/operator-vector-style.json`.
...
- workbench mini-map shows latest citizen location
- citizen location window map shows track history for movement review
...
Purpose:
- show a new incoming citizen request targeted to the current operator
...
- `incoming`: brand-new citizen request with answer/dismiss controls
...
Workbench layout:
- citizen/incident summary column
...
Citizen location:
- workbench shows latest citizen coordinates, accuracy, facing/heading, and elevation when available
- mini-map is clickable
- click opens a helper window component with a resizable/draggable MapLibre map
- window map renders latest location and citizen movement history
...
Current flow:
1. Producer records operator audio, citizen audio, and citizen video when applicable.
...
- `public/hotline.json`
- `public/maps/operator-vector-style.json`
...
- related call attempts, operator attempts, call sessions, participants, messages, attachments, media records, media chunks, incident type details, incident resources needed, transfers, team assignments, assignment resources, assignment notes, and citizen location history
...
- citizen location tracks for deleted incidents disappear
