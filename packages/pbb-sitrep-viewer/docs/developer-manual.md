...
`source_snapshot.rollup.hub_nodes[]`. Direct Hotline SITREPs also keep
`source_snapshot.rollup.hub_node` for compatibility. Viewer consumers should
prefer `hub_nodes[]` when they need a list of source hubs, and should not assume
that `source_snapshot.hub_node` is always top-level.
