...
Observed contracts:
- persistence and authorization now use `citizen_id` for public-user identity on current incident/call/session flows, and incident detail/location reads prefer `actual_citizen_*` / `citizen_location_*` columns; synchronized caller-shaped columns remain as rollback compatibility storage
