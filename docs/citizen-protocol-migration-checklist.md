...
- [~] Deploy latest citizen hardening changes before final storage drop: local WAMP runtime migrated and owner live testing passed through installed PWA terminal-status validation; latest branch now includes legacy config/model cleanup, legacy-schema guards, operator chat normalization after call end, and JSON map/settings updates, with production rollout/retest still required for the newest branch tip.
...
- [x] Remove caller database columns/tables after staged citizen migration and final decommission approval: Batch 5F now drops caller-shaped identity/detail/location storage in code, keeps citizen storage as canonical, and preserves only in-memory legacy read accessors plus request rejection tests. Local verification passed with `php artisan test tests\Feature` using PHP 8.2.29: 149 tests / 1028 assertions. Backup, deploy, smoke, monitoring, and rollback handling are captured in `docs/citizen-storage-5f-release-checklist.md`; production rollout still needs a live smoke test.

Current decommission gate:
- Caller-shaped database columns and the `incident_caller_locations` table have been removed in the forward 5F migration. Production deployment should still be treated as a destructive schema release using `docs/citizen-storage-5f-release-checklist.md` for backup and rollback readiness.
- The local and production-served readiness passes proved current `/citizen` flows are canonical, including live call setup, hangup/reconnect handling, operator offline recovery, and post-call status propagation. Legacy route and payload telemetry stayed flat through the live validation window. Realtime shared-service, Helper shared-service, installed PWA terminal-status, and durable storage/history scope confirmations are complete. Batch 1 route alias removal, Batch 2 Realtime event alias removal, Batch 3 request-field alias removal, Batch 4 PWA asset removal, and Batch 5A-F durable storage migration are complete in code.
