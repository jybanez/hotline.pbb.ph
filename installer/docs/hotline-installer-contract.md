...
- applies Account browser SSO settings from `hotline.pbb_account_sso_enabled`, `hotline.pbb_account_base_url`, `hotline.pbb_account_client_id`, `hotline.pbb_account_client_secret`, `hotline.pbb_account_redirect_uri`, `hotline.pbb_account_post_logout_redirect_uri`, `hotline.pbb_account_scopes`, and `hotline.pbb_account_ca_bundle` into the app-local `settings` table through `installer/bootstrap-runtime.php`; request-time Account SSO reads DB settings first and uses generic `PBB_ACCOUNT_*` environment values only as source/dev fallback.
- applies Account app-admin settings from `hotline.pbb_account_admin_api_enabled`, `hotline.pbb_account_admin_api_token`, and `hotline.pbb_account_admin_api_client` into the app-local `settings` table through `installer/bootstrap-runtime.php`; request-time app-admin middleware reads DB settings only and does not trust generic `PBB_ACCOUNT_ADMIN_API_*` environment values.
